...
/// that are grammar-legal (stops at first illegal token or end of draft).
...
pub fn dfa_validate_draft(
    ...
    for (i, &token) in tokens.iter().enumerate() {
        match dfa.advance(state, token) {
            ...
            None => return i, // token i is illegal
        }
    }
    tokens.len()
}
...
impl ModelRunner {
    ...
    pub fn run_mtp_decode(&self, seqs: Seqs) -> Result<Vec<Vec<u32>>> {
        ...
        let draft_tokens_gpu = if dfa_legal < draft_cpu.len() {
            crate::log_info!(
                "[MTP] DFA truncated draft: {} -> {} legal tokens (seq {})",
...
mod tests {
    use super::*;
    use llguidance::hw_dfa::{HwDfa, DfaEdge};

    fn test_dfa() -> HwDfa {
        // 3-state DFA: 0 --{1,2}--> 1 --{3}--> 2 (accept, self-loop on 0)
        HwDfa {
            num_states: 3,
            words_per_vob: 1,
            start_state: 0,
            mask_sign: vec![0, 0, 0],
            mask_words: vec![6, 8, 1], // state0: {1,2}, state1: {3}, state2: {0}
            edges: vec![
            ...
            state_labels: vec!["s0".into(), "s1".into(), "s2".into()],
        }
    }

    #[test]
    fn dfa_validate_all_legal() {
        let dfa = test_dfa();
        // 1 -> state1, 3 -> state2 (accept). Both legal.
        assert_eq!(dfa_validate_draft(Some(&dfa), 0, &[1, 3]), 2);
    }

    #[test]
    fn dfa_validate_rejects_illegal_token() {
        let dfa = test_dfa();
        // Token 0 is not allowed from state 0 (only 1,2 are).
        assert_eq!(dfa_validate_draft(Some(&dfa), 0, &[0, 1, 3]), 0);
        // Token 5 is not in the vocab (vocab=4, tokens 0-3).
        assert_eq!(dfa_validate_draft(Some(&dfa), 0, &[1, 5]), 1);
    }

    #[test]
    fn dfa_validate_none_dfa_returns_full_len() {
        assert_eq!(dfa_validate_draft(None, 0, &[1, 2, 3, 4]), 4);
    }

    #[test]
    fn dfa_validate_accept_state_self_loop() {
        let dfa = test_dfa();
        // From state 2 (accept), token 0 is allowed (self-loop via universal_target=2).
        assert_eq!(dfa_validate_draft(Some(&dfa), 2, &[0, 0, 0]), 3);
        // But token 1 is not allowed from state 2.
        assert_eq!(dfa_validate_draft(Some(&dfa), 2, &[0, 1]), 1);
    }

    /// Verify the truncation decision logic used in MTP/DFlash DFA validation.
    #[test]
    fn dfa_truncation_decision_logic() {
        let dfa = test_dfa();

        // All legal: no truncation
        let draft = vec![1, 3];
        let legal = dfa_validate_draft(Some(&dfa), 0, &draft);
        assert_eq!(legal, 2);

        // Partial: token 5 is illegal from state 2 (only 0 is allowed)
        let draft = vec![1, 3, 0, 0, 5];
        let legal = dfa_validate_draft(Some(&dfa), 0, &draft);
        assert_eq!(legal, 4); // 1->s1, 3->s2, 0->s2, 0->s2, 5=ILLEGAL

        // First token illegal: immediate rejection
        let draft = vec![0, 1, 3]; // 0 not allowed from state 0
        let legal = dfa_validate_draft(Some(&dfa), 0, &draft);
        assert_eq!(legal, 0);

        // No DFA: all tokens pass
        let draft = vec![99, 100, 101];
        let legal = dfa_validate_draft(None, 0, &draft);
        assert_eq!(legal, 3);
    }
}
