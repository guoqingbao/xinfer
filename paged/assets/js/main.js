/* ============================================================
   xInfer Documentation — Main JavaScript
   ============================================================ */
...
const SEARCH_INDEX = [
  { title: 'Home', url: 'index.html', desc: 'xInfer overview, features, and quick start', keywords: 'home overview features performance xinfer' },
  { title: 'Getting Started', url: 'getting-started.html', desc: 'Installation, prerequisites, and first run', keywords: 'install setup prerequisites cuda metal docker build npm pip' },
  { title: 'User Guide', url: 'user-guide.html', desc: 'Model loading, quantization, KV cache, multi-GPU, PD disaggregation', keywords: 'model gguf fp8 quantization kv cache multi-gpu turbo pd disaggregation mtp' },
  { title: 'Developer Guide', url: 'developer-guide.html', desc: 'Architecture, contributing, extending models', keywords: 'architecture scheduler pipeline contributing extend model rust crate' },
  { title: 'API Reference', url: 'api-reference.html', desc: 'CLI flags, REST endpoints, environment variables', keywords: 'cli api rest openai endpoint flag env structured outputs guided decoding' },
];
...
function initTypingAnimation() {
  ...
  const lines = [
    { text: '$ xinfer --m Qwen/Qwen3.6-27B-FP8 --kvcache-dtype turbo4 --ui-server', html: '<span class="token-comment">$</span> <span class="token-keyword">xinfer</span> <span class="token-flag">--m</span> <span class="token-string">Qwen/Qwen3.6-27B-FP8</span> <span class="token-flag">--kvcache-dtype</span> <span class="token-value">turbo4</span> <span class="token-flag">--ui-server</span>' },
    { text: '', html: '' },
    { text: '  Loading model weights... ████████████ 100%', html: '  Loading model weights... <span class="token-value">████████████</span> <span class="token-number">100%</span>' },
    { text: '  KV cache: turbo4, ~3.7x compression', html: '  KV cache: <span class="token-value">turbo4</span>, ~<span class="token-number">3.7x</span> compression' },
    { text: '  CUDA graphs captured for batch 1..32', html: '  CUDA graphs captured for batch <span class="token-number">1</span>..<span class="token-number">32</span>' },
    { text: '', html: '' },
    { text: '  ✓ API server ready at http://0.0.0.0:8000', html: '  <span class="token-value">✓</span> API server ready at <span class="token-string">http://0.0.0.0:8000</span>' },
    { text: '  ✓ Web UI ready at http://0.0.0.0:8001', html: '  <span class="token-value">✓</span> Web UI ready at <span class="token-string">http://0.0.0.0:8001</span>' },
  ];

  const CHAR_DELAY = 14;
  const LINE_PAUSE = 60;
  let lineIdx = 0, charIdx = 0, currentLines = [];
...
function initDownloads() {
  const grid = document.getElementById('dl-cards');
  const pipGrid = document.getElementById('pip-grid');
  if (!grid) return;

  const XINFER_VER = '0.13.8';
  ...
  const archs = [
    ...
    { sm: 'sm121', n: '121', name: 'SM121', gpu: 'DGX Spark / GB10',      cuda: 'CUDA 13', arch: 'arm64' },
  ];
  const metalArch = { sm: 'metal', n: 'metal', name: 'Metal', gpu: 'macOS Apple Silicon', cuda: '' };

  function renderCards(type) {
    grid.innerHTML = '';
    const items = archs.slice();
    if (type !== 'deb') items.push(metalArch);
    const lang = getLang();
    const dlText = lang === 'zh' ? '下载 →' : 'Download →';
    items.forEach(a => {
      const cpuArch = a.arch || 'x64';
      let fname;
      if (type === 'deb') {
        fname = 'xinfer-linux-' + cpuArch + '-' + a.sm + '.deb';
      } else if (type === 'tar') {
        fname = a.sm === 'metal'
          ? 'xinfer-' + XINFER_VER + '-darwin-arm64-metal.tar.gz'
          : 'xinfer-' + XINFER_VER + '-linux-' + cpuArch + '-' + a.sm + '.tar.gz';
      } else {
      ...
      el.href = DL_BASE + fname;
      el.target = '_blank';
      el.className = 'dl-card reveal';
      el.innerHTML =
        '<h4>' + a.name + ' — ' + a.gpu + '</h4>' +
        (a.cuda ? '<div class="dl-desc">' + a.cuda + '</div>' : '<div class="dl-desc">Apple Silicon</div>') +
        '<span class="dl-file">' + fname + '</span>' +
        '<span class="dl-action">' + dlText + '</span>';
      grid.appendChild(el);
    });
    initReveal();
  }
  ...
  document.querySelectorAll('.dl-tabs button').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('.dl-tabs button').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      renderCards(btn.getAttribute('data-dt'));
    });
  });

  if (pipGrid) {
    const sms = ['sm70', 'sm75', 'sm80', 'sm89', 'sm90', 'sm100', 'sm120', 'sm121', 'metal'];
    sms.forEach(v => {
      const cmd = 'pip install xinfer --index-url https://guoqingbao.github.io/xinfer/' + v + '/';
      const div = document.createElement('div');
      div.className = 'pip-item';
      div.innerHTML = '<code>' + cmd + '</code><button class="copy-btn" data-copy-text="' + cmd + '">Copy</button>';
